...
- `trnl (21, pc4` → `sas (21` ; `jmp pc4`. `trnl A,T` jumps if AC ≠ C(A); here against the literal `21` (FIODEC `/` — the slash, the spec's measure bar / title terminator, [*MusicCompiler-a.pdf*](../prs-docs/MusicCompiler-a.pdf), pp. 1, 8; earlier drafts rendered `21` as `|`). So: *if the typed character is **not** the bar `/`, fall through to treat it as a real keyword character* by jumping to `pc4`. The fall-through (character *is* `21`) hits the next line.
...
- **The decoded keywords match Samson's command set exactly.** All seventeen — `s l e end bass treble tenor alto units key rest copy up down h q tempo` — are documented commands in his spec: the articulation letters `s/l/e/h/q` (§I.B.6, §I.D.5), the four clefs (§I.D.3), `units` (§I.D.2), `key` (§I.D.4), `rest` (§I.D.6), `copy` (§I.D.7), `up`/`down` (§I.D.8), `tempo` (§I.D.9), `end` (§I.D.1), in [*MusicCompiler-a.pdf*](../prs-docs/MusicCompiler-a.pdf), pp. 6, 8–10. So the right-hand "meaning" column is sourced, not inferred; only the byte-level Flexowriter/IOT behavior remains not-emulator-verified. (`s`=staccato, `l`=legato, `e`=eighth/default, `h`=half, `q`=quarter; `bass`/`treble`/`tenor`/`alto` are the clefs.)
