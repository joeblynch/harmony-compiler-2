...
> **The full toolchain (per Peter Samson's [*music-workflow.pdf*](../prs-docs/music-workflow.pdf)).** (1) Break a score into ≤4 simultaneous voices. (2) Code each voice in the input language — documented in [*MusicCompiler-a.pdf*](../prs-docs/MusicCompiler-a.pdf) / [*MusicCompiler-b.pdf*](../prs-docs/MusicCompiler-b.pdf) (the "Music Compiler", also called the Harmony Compiler). (3) Type each voice in FIO-DEC; voices may share one tape, separated by `@` (the FIO-DEC "stop code", octal 13). (4) Run the compiler — RIM binary **`hc1d.rim`** — once **per voice**, appending the outputs; the result is the **"intermediate" format** (one 18-bit word per note), specified in [*music_intermediate_format.pdf*](../prs-docs/music_intermediate_format.pdf). (5) The player (`pdp1m16`, the successor to the `pdp1m13` documented next door) reads each voice and merges them into an internal **"final" format** in memory — two PDP-1 words per event holding 6-bit Soprano/Alto/Tenor/Bass pitches plus a 12-bit duration — and plays it. (Note the compiler's input spec says it "will handle selections taking from one to eight parts," while the intermediate format and player handle 1–4; treat ≤4 as the practical limit.) This doc set is reconciled against those Samson documents (`../prs-docs/`), cited inline.
