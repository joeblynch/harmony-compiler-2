...
The `nt` literals (`2 4 6 7 11 13 15 16 20 22 23 25 ...`, octal) are **pitch indices on the equal-tempered semitone scale** the player ultimately uses: consecutive staff positions a *whole step* apart differ by `2`, those a *half step* apart (the natural E-F and B-C boundaries) differ by `1` (e.g. `6 7` and `22 23`). So `nt[i]` encodes the diatonic major scale, and the builder's task is only to add or subtract `1` semitone at the positions named by the key signature. The semitone-code reading is confirmed by the intermediate format, whose 6-bit pitch field is exactly this scale — `0`/`1` = rest, `2` = C1, `3`–`76` octal = "equal-tempered notes by semitone (12 to the octave)", `77` = CS6 ([*music_intermediate_format.pdf*](../prs-docs/music_intermediate_format.pdf)). The `nt` values starting at `2` (= C1) line up with that field directly.
...
**Why:** `pva` classifies the key declaration into three families -- *natural* (C), *sharp*, *flat* -- and dispatches. This is exactly the spec's `key` syntax: **`key)`** = no sharps or flats, **`key(`N** = N sharps, **`key-`N** = N flats ([*MusicCompiler-a.pdf*](../prs-docs/MusicCompiler-a.pdf), p. 9 §I.D.4, Fig. 13). The `55`/`57`/`54` literals are the FIODEC codes for those glyphs — `)`/`(`/`-` — the transcriber types after `key` (matching the accidental characters in §I.B.5.b: `)` natural, `(` sharp, `-` flat). For the sharp/flat case the symbol is "instantly followed by a number which tells how many sharps or flats are in the key signature."
