...
**Verdict: the producer and consumer formats agree.** `pv4`/`ppp` emit, per section, a positive count word, N data words, and an arithmetic-`add` checksum word, three 6-bit lines per word, with `600000` as the final-bar marker and blank-line leader/gap/trailer — precisely the envelope [`../../pdp1m13/docs/05-data-formats.md` §1](../../pdp1m13/docs/05-data-formats.md) documents Music 13 as consuming. No format discrepancy was found between the two programs.
